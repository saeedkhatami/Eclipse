...
func _process(delta):
	...
	if is_instance_valid(true_player):
		...
		if spawn_timer < 0 and level['enemy_density'] > 0:
			...
			if randf() < (1 - (enemy_count/enemy_soft_cap) * .5):
				print("soft cap: " + str(enemy_soft_cap))
...
func start_game(mode, init_level, fixed_map_ = null, skip_transition = false):
		...
		elif mode == 'campaign': # This mode is plannde after Game off 23
			arcade_mode = false
